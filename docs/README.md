# Payroll documentation

This directory contains the full documentation for the Payroll application. **Documentation in this repo is licensed under [CC BY-SA 4.0](../LICENSE-DOCS).**

## Documentation index

| Document | Description |
|----------|-------------|
| [INSTALL.md](INSTALL.md) | Installation, server setup, and first run |
| [CONFIGURATION.md](CONFIGURATION.md) | Config options, environment, and paths |
| [API.md](API.md) | Full REST API reference |
| [API-QUICK-REFERENCE.md](API-QUICK-REFERENCE.md) | Condensed endpoint list and examples |
| [ADMIN.md](ADMIN.md) | Admin UI guide (login, dashboard, each page) |
| [DATA-MODEL.md](DATA-MODEL.md) | Database schema and tables |
| [TAX-CONFIG.md](TAX-CONFIG.md) | Tax bracket JSON format and validation |
| [PRD.md](PRD.md) | Product requirements document (authoritative) |

*Documentation in this repository is licensed under [CC BY-SA 4.0](../LICENSE-DOCS).*

## Quick links

- **New install?** Start with [INSTALL.md](INSTALL.md), then [CONFIGURATION.md](CONFIGURATION.md).
- **Integrating via API?** Use [API.md](API.md) or [API-QUICK-REFERENCE.md](API-QUICK-REFERENCE.md). A [Python SDK](../SDK/python/README.md) is available in `SDK/python/`.
- **Setting up tax rates?** See [TAX-CONFIG.md](TAX-CONFIG.md) and [PRD.md](PRD.md) §6.
